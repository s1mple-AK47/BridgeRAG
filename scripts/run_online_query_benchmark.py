...
def load_questions_from_final_filtered(file_path: Path, limit: int) -> List[Dict]:
    """从 final_filtered_questions.jsonl 文件中加载问题数据。"""
    questions_data = []
...
def main(output_file_path: str = None):
    ...
    # 更改输入文件为 final_filtered_questions.jsonl
    data_file = project_root / "final_filtered_questions.jsonl"
    
    # Use provided output file path or default
    if output_file_path:
        output_file = Path(output_file_path)
    else:
        output_file = project_root / "logs" / "online_query_results.jsonl"
    ...
    # 使用新的数据加载函数
    all_questions_data = load_questions_from_final_filtered(data_file, limit=QUESTION_LIMIT)
    ...
    with open(output_file, 'a', encoding='utf-8') as f:
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            ...
            progress = tqdm(as_completed(future_to_question), total=len(questions_to_process), desc="在线查询基准测试")
